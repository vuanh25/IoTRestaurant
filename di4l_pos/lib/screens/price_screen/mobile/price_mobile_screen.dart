...
class PriceMobileScreen extends StatefulWidget {
  /// MARK: - Initials;
  static BlocProvider<PriceCubit> provider() {
    return BlocProvider(
      create: (context) => PriceCubit(),
      child: PriceMobileScreen(),
    );
  }

  const PriceMobileScreen({Key? key}) : super(key: key);

  @override
  State<PriceMobileScreen> createState() => _PriceMobileState();
}

class _PriceMobileState extends State<PriceMobileScreen> with AfterLayoutMixin {
  /// MARK: - Initials;
